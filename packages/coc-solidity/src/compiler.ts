...
export class Compiler {
  ...
  constructor(solcCachePath: string) {
    ...
    public async compile(
      contracts: any,
      diagnosticCollection: coc.DiagnosticCollection,
      buildDir: string,
      rootDir: string,
      ...
      if (Object.keys(contracts).length === 0) {
        coc.workspace.showMessage('No solidity files (*.sol) found');
        return null;
      }
      return new Promise((resolve, reject) => {
        this.initialiseCompiler().then(() => {
          try {
            ...
            coc.workspace.showMessage(reason);
            reject(reason);
          }
        });
      });
    ...
    private outputErrorsToChannel(outputChannel: coc.OutputChannel, errors: any) {
    ...
    private async initialiseCompiler(): Promise<void> {
      const rootFolder = coc.workspace.workspaceFolders[0]
      ...
      this.outputChannel.clear();
      this.outputChannel.show();
      const remoteCompiler = coc.workspace.getConfiguration('solidity').get<string>('compileUsingRemoteVersion');
      const localCompiler = coc.workspace.getConfiguration('solidity').get<string>('compileUsingLocalVersion');
      const enableNodeCompiler = coc.workspace.getConfiguration('solidity').get<boolean>('enableLocalNodeCompiler');
    ...
    private processCompilationOutput(
      outputString: any,
      outputChannel: coc.OutputChannel,
      diagnosticCollection: coc.DiagnosticCollection,
      buildDir: string,
      sourceDir: string,
      excludePath?: string,
      ...
      if (Object.keys(output).length === 0) {
        ...
        coc.workspace.showMessage(noOutputMessage);
        // vscode.window.setStatusBarMessage(noOutputMessage); // TODO: Port - is this portable?
      ...
      if (output.errors) {
        ...
        if (errorWarningCounts.errors > 0) {
          ...
          coc.workspace.showMessage(compilationWithErrorsMessage);
          ...
          if (errorWarningCounts.warnings > 0) {
            coc.workspace.showMessage(`Compilation had ${errorWarningCounts.warnings} warnings`);
          }
          ...
          coc.workspace.showMessage(compilationWithWarningsMessage);
        ...
        coc.workspace.showMessage(compilationSuccessMessage);
    ...
    private writeCompilationOutputToBuildDirectory(output: any, buildDir: string, sourceDir: string,
      ...
      const rootUri = coc.Uri.parse(coc.workspace.workspaceFolders[0].uri)
